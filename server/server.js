...
const express = require('express');
    ...
    userSchema.set('toJSON', { virtuals: true, transform: (doc, ret) => { delete ret._id; delete ret.__v; if(ret.locationId) { ret.location = ret.locationId; delete ret.locationId; } } });
    ...
    batchSchema.set('toJSON', { virtuals: true, transform: (doc, ret) => { delete ret._id; delete ret.__v; if(ret.locationId) { ret.location = ret.locationId; delete ret.locationId; } } });
    ...
    invoiceSchema.set('toJSON', { 
        ...
        transform: (doc, ret) => { 
            delete ret._id; 
            delete ret.__v;
            // Check for populated student and only include necessary fields
    ...
    async function startServer() {
        ...
        const corsOptions = {
            origin: (origin, callback) => {
                // For development, we are very permissive to avoid issues with different ports.
                // Allow requests with no origin (like mobile apps or curl)
                if (!origin) return callback(null, true);
                // Allow all whitelisted origins
        ...
        const getFamilyMemberIds = async (sessionUser) => {
            // For teachers, they only see content assigned to them
            if (sessionUser.role === 'Teacher') {
                return [sessionUser.id];
            }
            
            // For students/parents, find all accounts with the same base email
            ...
            // Always include the main account holder's ID
        ...
        app.post('/api/register', async (req, res) => {
            ...
            try {
                ...
                for (const userData of usersData) {
                    ...
                    if (restOfUserData.email.toLowerCase() === 'admin@nadanaloga.com') {
                        restOfUserData.role = 'Admin';
                    }
                    ...
                    // If a new student registers, create notifications
                    if (user.role === 'Student' && adminUser) {
                        ...
                        if (mailTransporter) {
                            ...
                            // Send mail outside the transaction loop if needed for performance
        ...
        app.get('/api/session', (req, res) => {
            if (req.session.user) {
                ...
                // Return 200 with a null body to indicate no active session.
                // This prevents the browser from showing a red 401 error in the console
                // for the expected case of a user not being logged in.
        ...
        app.put('/api/profile', ensureAuthenticated, async (req, res) => {
            try {
                ...
                // If the user tries to update their own profile, we should use their actual ID
                ...
                // Only update the session if the user is updating their own primary profile
        ...
        app.get('/api/student/enrollments', ensureAuthenticated, async (req, res) => {
            try {
                ...
                const studentEnrollmentDetails = enrolledBatches.map(batch => {
                    // Filter schedule items to only those the student is in for this batch
        ...
        const createCrudRoutes = (path, model, modelName) => {
            // Public GET all - MODIFIED for targeted content
            ...
            try {
                ...
                if (users.length > 0) {
                    ...
                    if (mailTransporter) {
                        for (const user of users) {
                            ...
                            // Not awaiting this intentionally to not block the response
                ...
                if (sendWhatsApp) {
                    ...
                    for (const user of users) {
                        ...
                        if (phoneNumber) {
                            // In a real app, you'd call the WhatsApp Business API here
