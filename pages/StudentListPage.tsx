...
import { GRADES, WEEKDAYS, TIME_SLOTS } from '../../constants';
...
const StudentListPage: React.FC = () => {
    ...
    const allSchedules = useMemo(() => WEEKDAYS.flatMap(day => TIME_SLOTS.map(slot => `${day} ${slot}`)), []);
    ...
    const filteredAndSortedStudents = useMemo(() => {
        ...
        processStudents = processStudents.filter(student => {
            ...
            if (scheduleFilter && !student.schedules?.some(s => s.timing === scheduleFilter)) return false;
            if (searchQuery) {
                ...
                const searchIn = [
                    ...
                    ...(student.courses || []),
                    ...(student.schedules?.map(s => s.timing) || [])
    ...
    }, [students, searchQuery, gradeFilter, courseFilters, scheduleFilter, sortConfig]);
    ...
    const handleStudentSaveChanges = async (payload: { user: User; batchChanges: Map<string, { oldBatchId?: string; newBatchId: string; }> }) => {
        const { user, batchChanges } = payload;
        ...
        if (batchChanges.size > 0) {
            ...
            batchChanges.forEach((change, course) => {
                const oldBatchName = batches.find(b => b.id === change.oldBatchId)?.name || "Unassigned";
                const newBatchName = batches.find(b => b.id === change.newBatchId)?.name || "Unassigned";
        ...
        if (window.confirm(confirmMsg)) {
            try {
                ...
                batchChanges.forEach((change) => {
                    const { oldBatchId, newBatchId } = change;
                    
                    if (oldBatchId && oldBatchId !== newBatchId) {
                        const existingOldBatch = updatedBatchesMap.get(oldBatchId) || batches.find(b => b.id === oldBatchId);
                        if (existingOldBatch) {
                    ...
                    if (newBatchId && newBatchId !== 'unassigned' && newBatchId !== oldBatchId) {
                        const existingNewBatch = updatedBatchesMap.get(newBatchId) || batches.find(b => b.id === newBatchId);
                        if (existingNewBatch) {
                             const newBatch: Batch = JSON.parse(JSON.stringify(existingNewBatch));
                            newBatch.schedule.forEach(s => {
                                if (!s.studentIds.includes(user.id)) {
                ...
                updatedBatchesMap.forEach((batchToUpdate, batchId) => {
                    batchUpdatePromises.push(updateBatch(batchId, batchToUpdate));
                });
    
                const [savedUser, ...savedBatches] = await Promise.all([updatedUserPromise, ...batchUpdatePromises]);
                
                setAllUsers(prev => prev.map(u => u.id === savedUser.id ? savedUser : u));
                
                const newBatchesMap = new Map(batches.map(b => [b.id, b]));
                savedBatches.forEach(b => newBatchesMap.set(b.id, b));
                setBatches(Array.from(newBatchesMap.values()));
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-md my-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div className="lg:col-span-2">
                                <label htmlFor="schedule-filter" className="text-sm font-medium text-gray-700">Schedule</label>
                                <select id="schedule-filter" value={scheduleFilter} onChange={e => setScheduleFilter(e.target.value)} className="w-full form-select mt-1">
                                    <option value="">All Schedules</option>
                                    {allSchedules.map(s => <option key={s} value={s}>{s}</option>)}
                                </select>
                            </div>
                        </div>
                    </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <SortableHeader columnKey="id" title="Student ID" sortConfig={sortConfig} requestSort={requestSort} />
                                            ...
                                            <SortableHeader columnKey="dateOfJoining" title="Date Joined" sortConfig={sortConfig} requestSort={requestSort} />
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {filteredAndSortedStudents.length > 0 ? filteredAndSortedStudents.map((user) => (
                                            <tr key={user.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{user.id.slice(-6).toUpperCase()}</td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.dateOfJoining ? new Date(user.dateOfJoining).toLocaleDateString() : 'N/A'}</td>
                                                <td className="px-6 py-4 text-sm text-gray-500">
                                                    {user.schedules && user.schedules.length > 0 ? (
                                                        <div className="space-y-2">
                                                            {user.schedules.map(s => {
                                                                const teacherName = s.teacherId ? teachersMap.get(s.teacherId)?.name : 'Unassigned';
                                                                const course = courses.find(c => c.name === s.course);
                                                                return (
                                                                    <div key={`${user.id}-${s.course}-${s.timing}`}>
                                                                        <div className="flex items-center space-x-1.5">
                                                                            {course && <Tooltip content={s.course} position="top">
                                                                                <CourseIcon iconName={course.icon} className="h-5 w-5 text-brand-secondary" />
                                                                            </Tooltip>}
                                                                            <span className="font-medium text-gray-700">{s.timing || 'No time set'}</span>
                                                                        </div>
                                                                        {s.teacherId && (
                                                                            <div className="text-xs text-indigo-500 pl-7 -mt-0.5">
                                                                                by {teacherName}
                                                                            </div>
                                                                        )}
                                                                    </div>
